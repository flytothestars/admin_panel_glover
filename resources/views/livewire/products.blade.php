...
<div>
    ...
    <div x-data="{ open: @entangle('showCreate') }">
        <x-modal-lg confirmText="{{ __('Save') }}" action="save" :clickAway="false">
            ...
            <livewire:component.multiple-media-upload title="{{ __('Image') }}" name="photos" types="PNG or JPEG" fileTypes="image/*" emitFunction="photoSelected" max="{{ setting('filelimit.max_product_images', 3) }}" maxSize="{{ setting('filelimit.product_image_size', 200) }}" />
            ...
            <div class="p-2 my-4 border rounded">
                <x-checkbox title="{{ __('Digital') }}" name="digital" description="{{ __('If product is digital and can be downloaded') }}" :defer="false" />
                @if ($digital)
                <x-input.filepond wire:model="digitalFile" allowImagePreview="false" allowFileSizeValidation="true" maxFileSize="{{ setting('filelimit.max_product_digital_files_size', 2) }}mb" />
                @endif
            </div>
    ...
    <div x-data="{ open: @entangle('showEdit') }">
        <x-modal-lg confirmText="{{ __('Update') }}" action="update">
            ...
            <livewire:component.multiple-media-upload title="{{ __('Image') }}" name="photos" types="PNG or JPEG" fileTypes="image/*" emitFunction="photoSelected" :preview="$photos" max="{{ setting('filelimit.max_product_images', 3) }}" maxSize="{{ setting('filelimit.product_image_size', 200) }}" />
            ...
            <x-details.item title="{{ __('File') }}" text="">
                <div class="space-y-3">
                    ...
                    <div class="flex items-center p-2 border rounded">
                        ...
                        <a href="{{ $file->link }}" target="_blank" class="font-medium hover:underline text-primary-500 hover:text-primary-800 hover:font-bold">
                            {{ __('Download') }}
                        </a>
                    </div>
                    @endforeach
